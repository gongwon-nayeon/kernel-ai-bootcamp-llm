...
def generate(state: State):
    ...
    <rules>
    - context에 제공된 문서의 출처(문서의 파일명, 페이지 번호)를 언급하며 답변을 작성하세요.
...
def decide_to_generate(state: State) -> Literal["transform_query", "generate"]:
    ...
    if state["document"] == "" and (not state["retry_num"] >= 2):
...
def grade_generation_v_documents_and_question(state: State) -> Literal["useful", "not useful", "not supported"]:
    ...
    # 환각 평가
    llm = _get_llm()
    hallucination_grader = llm.with_structured_output(GradeHallucinations) # yes or no
    ...
    if grade == "yes":  # 환각 없음
        ...
        # 답변 유용성 평가
        llm = _get_llm()
        answer_grader = llm.with_structured_output(GradeAnswer) # yes or no
        ...
        if grade == "yes":  # 유용함
            ...
            if state["retry_num"] >= 2:
                print("---DECISION: MAX RETRIES REACHED, USE WEBSERACH---")
                return "websearch"
            return "not useful"
    else:  # 환각 발생
        print("---DECISION: GENERATION IS NOT GROUNDED IN DOCUMENTS, RE-TRY---")
        if state["retry_num"] >= 2:
            print("---DECISION: MAX RETRIES REACHED, ACCEPT GENERATION---")
            return "websearch"
...
def web_search(state: State):
    """
    웹 검색을 수행하고 검색 결과를 기반으로 답변을 생성합니다.
    """
    print("##### WEB SEARCH #####")
    ...
    SYSTEM_PROMPT = """
    당신은 웹 검색 결과를 기반으로 답변하는 어시스턴트입니다.
    주어진 웹 검색 결과를 활용하여 사용자의 질문에 대해 정확하고 유용한 답변을 제공하세요.

    <rules>
    - 검색 결과의 출처(URL)를 명시하며 답변하세요.
    - 답변은 마크다운 문법 형식으로 가독성 좋게 작성하세요.
    - 여러 검색 결과를 종합하여 답변하세요.
    </rules>
    """
